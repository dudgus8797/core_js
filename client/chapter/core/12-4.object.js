...
const shopOrder = {
  ...
  menu: [
    ...
    { name: '곰곰 주꾸미', price: 17000, count: 5 },
  ],
  totalPrice() {
    return this.menu.reduce((acc, cur) => acc + cur.price * cur.count, 0);
  },
};
// console.log (shopOrder.menu[0].price * shopOrder.menu[0].count) + (shopOrder.menu[1].price *shopOrder.date[1].count);
// let total = 0;
// shopOrder.menu.forEach((item) => {
//   total += item.price * item.count;
// });

// shopOrder.menu.reduce((acc, cur) => {
//   acc + cur.price * cur.count;
// }, 0);

// let total = shopOrder.menu.reduce((acc, cur) => acc + cur.price * cur.count, 0);
...
// 너 item에 id랑 text랑 link만 넣어야 한다라고 제한을 줘야함
// type Item = {
//   id:string,
//   text: String,
//   link: string
